...
document.addEventListener('DOMContentLoaded', () => {
  if (document.querySelector('[data-select]')) {
    ...
    const selectOptionsMobile = document.querySelectorAll(
      '.custom-options-mobile li'
    );
    const selectOptionsCity = document.querySelectorAll(
      '.custom-options-city li'
    );
    ...
    const maxSumSubscription = document.querySelectorAll(
      '.rent-card-mobile-subscription'
    )[0];
    const citySubscription = document.querySelectorAll(
      '.rent-card-city-subscription'
    )[0];
    ...
    const selectOption = (target) => {
      ...
      if (target) {
        ...
        const selectInput = document.querySelector(
          `[data-input="${select.getAttribute('data-select')}"]`
        );
        const selectField = select.querySelector('.custom-select-value');
        const selected = target.getAttribute('data-value');
        const selectFieldImage = select.querySelector(
          '.custom-select-value-img img'
        );
        ...
        if (target.dataset.url) {
          ...
          const audio = select.querySelector(
            `[data-origin="${target.dataset.url}"]`
          );
          if (audio) {
        ...
        if (target.dataset.langs) {
          ...
          const langSelectField = langSelect.querySelector(
            '.custom-select-value'
          );
          const langSelectInput = document.querySelector(
            `[data-input="${langSelect.getAttribute('data-select')}"]`
          );
        ...
        if (select.getAttribute('data-select')) {
          if (
            document.querySelector(`.${select.getAttribute('data-select')}`)
          ) {
            let parent = document.querySelector(
              `.${select.getAttribute('data-select')}`
            );
            if (parent.querySelector('.rent-card-possibility')) {
              parent.querySelector('.rent-card-possibility').textContent =
                target.dataset.possibility || 'Нет данных';
            }
            if (parent.querySelector('.rent-card-time')) {
              parent.querySelector('.rent-card-time').textContent =
                target.dataset.time || 'Нет данных';
            }
            if (parent.querySelector('.rent-card-minute')) {
              parent.querySelector('.rent-card-minute').textContent =
                target.dataset.minute || 0;
            }
            parent.querySelector('.rent-card-pricing').textContent =
              target.dataset.pricing || 0;
            parent.querySelector('.rent-card-subscription').textContent =
              target.dataset.subscription || 0;
          }
        }
...
function sendCityCallerRequest(form) {
  sendAjaxForm(form).then((response) => {
    if (response.ok) {
      $(form).replaceWith(
        `<div style="display: block; width: 400px"><span style="color: white">${prettyNumberSuccess}</span></div>`
      );
    } else {
...
function callBackRequest(form) {
  sendAjaxForm(form).then((response) => {
    if (response.ok) {
      $(form).replaceWith(
        `<div class="claim-success">${callMeBackSuccess}</div>`
      );
    } else {
      parsingFormErrors(form, response);
    }
...
function startMainDemo(event) {
  ...
  sendAjaxForm(form).then((response) => {
    if (response.ok) {
      ...
      const demoProcessStatuses = demoProcess.querySelector(
        '#demoProcessStatuses'
      );
      demoProcessStatuses.replaceChildren(...[]);
      let processStatus = createTagElement(
        'li',
        'presentation-block-step',
        'Робот уже звонит вам'
      );
...
async function startDemo(
  ...
  const phone = form.elements
    ...
    .value.replace('(', '')
    .replace(')', '');
  const demoComplete = demoBlock.querySelector(
    '[data-demo-step="demoComplete"]'
  );
  ...
  while (
    demoStatus.call_status !== 'complete' &&
    demoStatus.call_status !== 'err'
  ) {
    ...
    if (demoStatus.actions && !arrayEquals(actions_state, demoStatus.actions)) {
      let processStatus = createTagElement(
        'li',
        'presentation-block-step',
        demoStatus.actions[demoStatus.actions.length - 1]
      );
      demoProcessStatuses.appendChild(processStatus);
...
function placeThumbnail() {
  ...
  if (video) {
    var content =
      video.dataset.content !== 'None'
        ? `"contentUrl": ${video.dataset.content},`
        : '';
    $('#youtube-div').append(`
...
function loadVideo(video_id) {
  document.getElementById(
    'video-div'
  ).innerHTML = `<iframe class="youtube-video" src="https://www.youtube.com/embed/${video_id}?rel=0&autoplay=1&modestbranding=1&controls=2" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>`;
}
